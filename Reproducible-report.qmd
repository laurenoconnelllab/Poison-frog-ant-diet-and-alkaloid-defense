...
All data sets used for this analyses can be accessed in: <https://github.com/laurenoconnelllab/Poison-frog-ant-diet-and-alkaloid-defense>
...
allpop.chord <- alkadiver %>% 
  ...
  group_by(population) %>%
  group_split() %>%
  set_names(map_chr(., ~ as.character(unique(.x$population)))) %>%
  map(~ .x %>%
        select(-population) %>%
...
## S. Domingo
S.domingo =allpop.chord$s_domingo
